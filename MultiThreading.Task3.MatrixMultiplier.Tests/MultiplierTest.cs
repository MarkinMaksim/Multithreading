...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        {
            var count = 51;
            var m1 = new Matrix(count, count, true);
            var m2 = new Matrix(count, count, true);
            ...
            //Check classic method
            stopWatch.Start();
            multiplier.Multiply(m1, m2);
            stopWatch.Stop();
            var result = stopWatch.Elapsed;
            Console.WriteLine($"Rasult: {result.TotalMilliseconds}");

            parallelStopWatch.Start();
            parallelMultiplier.Multiply(m1, m2);
            parallelStopWatch.Stop();
            var parallelResult = parallelStopWatch.Elapsed;

            Console.WriteLine($"Rasult of parallel: {parallelResult.TotalMilliseconds}");

            //Effective size is 51
        }
